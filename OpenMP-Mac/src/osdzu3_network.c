...
#ifndef OSDZU3_ENABLE_OPENMP_PIPELINE
#define OSDZU3_ENABLE_OPENMP_PIPELINE 0
#endif
...
static float osdzu3_backward_to_buffers(osdzu3_network_t *network,
                                        uint32_t label,
                                        bool use_cached_state,
    ...
    const float *current_output = network->activations[network->config.network.layer_count];
    const float *cached_output = osdzu3_cache_activation_ptr(network, label, network->config.network.layer_count);

    for (i = 0; i < output_count; i++) {
        float target = (i == label) ? 1.0f : 0.0f;
        float delta = current_output[i] - target;
        delta_buffers[network->config.network.layer_count - 1U][i] = delta;
        total_delta += fabsf(current_output[i] - cached_output[i]);
    }
    ...
    for (layer = (int) network->config.network.layer_count - 1; layer >= 0; layer--) {
        ...
        for (src_index = 0; src_index < src; src_index++) {
            for (dst_index = 0; dst_index < dst; dst_index++) {
                size_t idx = (size_t) src_index * dst + dst_index;
                target_weights[layer][idx] += src_activations[src_index] * delta_buffers[layer][dst_index];
            }
        }
        for (dst_index = 0; dst_index < dst; dst_index++) {
            target_biases[layer][dst_index] += delta_buffers[layer][dst_index];
        }
        ...
        if (layer > 0) {
            ...
            for (src_index = 0; src_index < src; src_index++) {
                ...
                for (dst_index = 0; dst_index < dst; dst_index++) {
                    size_t idx = (size_t) src_index * dst + dst_index;
                    sum += network->weights[layer][idx] * delta_buffers[layer][dst_index];
                }
                delta_buffers[layer - 1][src_index] =
                    sum * osdzu3_activation_derivative(network->config.network.layers[layer - 1].activation,
                                                       prev_preactivations[src_index],
                                                       prev_activations[src_index]);
            }
    ...
    return output_count ? (total_delta / (float) output_count) : 0.0f;
}
    ...
    for (epoch = 0; epoch < epoch_count; epoch++) {
        ...
        for (i = 0; i < sample_count; i++) {
            ...
            if (!network->config.network.speculative_enabled || !network->cache_valid[label]) {
                ...
                float average_delta;
#if OSDZU3_ENABLE_OPENMP_PIPELINE
                ...
                #pragma omp parallel num_threads(2)
                {
                    ...
                    #pragma omp single
                    {
                        average_delta = osdzu3_backward_to_buffers(network,
                                                                  label,
                                                                  true,
                                                                  network->spec_grad_weights,
                                                                  network->spec_grad_biases,
                                                                  network->deltas);
                    }
                    #pragma omp barrier
                }
                ...
                loss_total += osdzu3_compute_loss(network, label);
                if (average_delta <= network->config.network.speculative_threshold) {
